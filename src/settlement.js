...
export function buildSettlement(M) {
  ...
  for (const [list, base] of [[HAMLET, 40], [FARM, 50]]) for (const [i, hp] of list.entries()) {
    ...
    g.add(h.group);
    houses.push({ name: hp.name, group: h.group, glowMats: hp.glow === false ? [] : h.glowMats, sliders: h.sliders || [], params: { ...hp }, pos: new THREE.Vector3(hp.cx, y, hp.cz), w: hp.w, d: hp.d });
